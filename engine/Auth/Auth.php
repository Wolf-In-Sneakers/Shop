...
class Auth
{
    ...
    public function registration(string $login, string $passwd, string $check_passwd, string $name): bool
    {
        ...
        if (!$this->mysqli->query($sql_query))
            throw new Exception("Неудалось выполнить запрос на добавление пользователя в базу данных!");
        ...
        if ($row = $answer->fetch_assoc()) {
            if (password_verify($passwd, $row['password'])) {
                if (session_start()) {
                    ...
                    if (!$this->mysqli->query($sql_query)) {
                        throw new Exception("Неудалось выполнить запрос на обновление данных пользователя!");
                    }
                    ...
                    $sql_query = "SELECT p.id_product, p.name, price, b.quantity, i.name as image_name FROM basket b
                                    ...
                                    ORDER BY b.created;";
                    if (!($answer = $this->mysqli->query($sql_query))) {
                        throw new Exception("Неудалось выполнить запрос на выбоку товаров из базы данных!");
                    }
